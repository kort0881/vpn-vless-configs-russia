...
# Определяем корень репозитория (папка, содержащая scripts/)
REPO_ROOT = Path(__file__).parent.parent
# Файл конфигурации, который читает mirror.py
CONFIG_SOURCES_FILE = REPO_ROOT / "data" / "config_sources.json"
# Для совместимости с кодом, если используется URLS_BASE (пустой список)
URLS_BASE = []

MANIFEST_URL = "https://github.com/cook369/proxy-collect/raw/refs/heads/main/dist/manifest.json"
OUT_FILE = Path(__file__).parent / "proxy_collect_only_new_for_mirror.txt"
OUT_EXTRA_FILE = Path(__file__).parent / "proxy_collect_extra_sources.txt"
